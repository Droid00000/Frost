...
module AutoPinArchiver
  ...
  unknown(type: :CHANNEL_PINS_UPDATE) do |event|
    channel = bot.channel(event.data['channel_id']&.to_i, server = event.data['guild_id']&.to_i)
    pins = channel.pins
    ...
    if pins.count == 50 && archiver_records(server: event.data['guild_id']&.to_i, type: :check)
      ...
      if message.attachments.any?
        archive_channel.send_embed do |embed|
          ...
          embed.author = Discordrb::Webhooks::EmbedAuthor.new(name: message.author.username.to_s, url: message.link.to_s, icon_url: message.author.avatar_url.to_s)
