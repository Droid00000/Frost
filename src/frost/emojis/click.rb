...
def button_click(data)
  ...
  if data.server.emoji_limit?(data.get_component(data.custom_id).emoji)
  ...
  data.update_message do |builder, components|
    components.row do |buttons|
      data.message.to_message.buttons.each do |old_button|
        buttons.button(style: old_button.custom_id == data.custom_id ? 4 : old_button.style,
                       emoji: old_button.emoji.id, custom_id: old_button.custom_id)
      end
    end

    builder.add_embed do |embed|
      embed.description = EMBED[28]
      embed.color = UI[5]
    end
  end

  emoji = data.server.add_emoji(data.get_component(data.custom_id).emoji.name,
                                data.get_component(data.custom_id).emoji.file)
