...
def members_back(data)
  ...
  members, id, roles = [], data.custom_id.scan(/\d+/).first.to_i, Frost::Houses.cult(data).members

  roles.take(id * 30).each_with_index do |user, count|
      ...
      builder.add_embed do |embed|
        components.row do |component|
          ...
          component.button(style: 1, label: EMBED[183], emoji: EMBED[190],
                           custom_id: format(EMBED[191], id + 1)) unless (id > members.size)
        end
      end
    end
  else
