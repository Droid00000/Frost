...
module Emojis
  ...
  def self.cache(data)
    ...
    if data.is_a?(Discordrb::Events::ReactionAddEvent)
      return if data.bot.emoji(data.emoji.id).nil?
  ...
  def self.stats(data)
    ...
    # audit job here in order to update results on demand. Do this after
    # actually responding to the command for performance reasons.
    Storage.large?
  end
end
