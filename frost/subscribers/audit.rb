...
Rufus::Scheduler.s.cron "0 0 * * *" do
  Boosters::Storage.state.chunks.each do |chunk|
    BOT.gateway.members(chunk[:guild_id], chunk[:users])
  end

  tombstones = []

  Boosters::Storage.state.list_boosters do |user|
  ...
  Boosters::Storage.state.delete_boosters(tombstones) if tombstones.any?
end
