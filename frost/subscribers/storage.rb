...
module Boosters
  ...
  class Member
    ...
    alias color color_id

    # @return [Guild, Integer]
    attr_reader :guild_id
    alias guild guild_id
    ...
    def initialize(data)
      ...
      from_query(find_guild_booster(*@query.values.map(&:to_i)))
    end
    ...
    def from_query(*options)
      @banned = options[0][:banned]
      @role_id = options[0][:user_role]
      @color_id = options[0][:color_id]
      @guild_id = Guild.new(@data, **options[0].slice(:role_id, :any_icon))
    end

    # @!visibility private
    def find_guild_booster(*options)
      POSTGRES.transaction do
        @lazy ? {} : POSTGRES["SELECT * FROM booster_metadata(?, ?)", *options].first || {}
      end
    end
  end
