...
# def process_response(user, bucket)
#   return if bucket.messages.empty?

#   primary_description = if !user.respond_to?(:joined_at)
#                           format(RESPONSE[3], bucket.messages.length, user.id)
#                         else
#                           format(RESPONSE[3], bucket.messages.length, user.id, user.joined_at.to_i)
#                         end

#   # We're going to have to create our file to upload inside of a temporary directory.
#   Dir.mktmpdir do |directory|
#     # The path to the file to create.
#     path = File.join(directory, "attachment-urls.txt")

#     # Write the data to the file here, and then close it.
#     File.open(path, "w") { it.puts(bucket.files.map(&:url).join("\n\n")) }

#     channel = BOT.channel(CONFIG[:Moderator][:CHANNEL])
