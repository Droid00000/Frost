...
-- Function for fetching an event role.
CREATE OR REPLACE FUNCTION event_user (user_id BIGINT, role_id BIGINT)
  RETURNS TABLE (
    role_id BIGINT,
    guild_id BIGINT,
    permission BOOLEAN
...
SELECT
   event_roles.role_id,
   event_roles.guild_id,
   COALESCE(event_users.user_id IS NOT NULL, FALSE) AS permission
FROM
  event_roles LEFT JOIN event_users
    ON event_users.user_id = $1 AND event_users.role_id = $2
  WHERE event_roles.role_id = $2
$$;