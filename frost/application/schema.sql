...
CREATE OR REPLACE FUNCTION booster_metadata (user_id BIGINT, guild_id BIGINT)
  RETURNS TABLE (
    color_id BIGINT, 
    role_id BIGINT, 
    any_icon BOOLEAN, 
...
FROM
   ...
   LEFT JOIN
      booster_settings 
      ON guild_boosters.guild_id = booster_settings.guild_id 
   LEFT JOIN
      banned_boosters 
      ON guild_boosters.user_id = banned_boosters.user_id 
      AND guild_boosters.guild_id = banned_boosters.guild_id 
WHERE
  (guild_boosters.user_id = $1) AND (guild_boosters.guild_id = $2) LIMIT 1;
$$;
